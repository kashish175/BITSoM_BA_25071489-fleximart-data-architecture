...
GROUP BY 
    d.year, 
    d.quarter, 
    d.month,      -- Using numeric month for correct chronological ordering
    d.month_name
ORDER BY 
    d.year, 
    d.quarter, 
...
SELECT
    ...
    -- Window function to calculate % of total revenue across all products
    CONCAT(
        ROUND((SUM(f.total_amount) / SUM(SUM(f.total_amount)) OVER()) * 100, 1),
